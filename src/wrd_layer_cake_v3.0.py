...
def main(opt):
    ...
    model_name, model_path = get_model_identifier(opt.pretrained)
    print("model_name:", model_name)
    print("model_path:", model_path)

    if (opt.pretrained == 'glove'):
        opt.glove_path = model_path
    elif (opt.pretrained == 'word2vec'):
        opt.word2vec_path = model_path
    elif (opt.pretrained == 'fasttext'):
        opt.fasttext_path = model_path
    elif (opt.pretrained == 'bert'):
        opt.bert_path = model_path
    elif (opt.pretrained == 'roberta'):
        opt.roberta_path = model_path
    elif (opt.pretrained == 'distilbert'):
        opt.distilbert_path = model_path
    elif (opt.pretrained == 'xlnet'):
        opt.xlnet_path = model_path
    elif (opt.pretrained == 'gpt2'):
        opt.gpt2_path = model_path
    elif (opt.pretrained == 'llama'):
        opt.llama_path = model_path
    ...
    already_modelled, logfile, method_name, pretrained, embeddings, embedding_type, emb_path, lm_type, mode, system = initialize_testing(opt, model_name, program=program, version=version)
