...
class LCTFIDFVectorizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, tokenized_documents, original_documents=None):
        ...
        if (tokenized_documents is None):
            raise ValueError("tokenized_documents is None")
        ...
        if (self.debug):
            ...
            if (original_documents is not None):
                print("original_documents: ", type(original_documents), len(original_documents))
                print("original_documents[0]: ", type(original_documents[0]), original_documents[0])
            else:
                print("original_documents == None")
...
def compute_tce_matrix(vocabsize, vectorized_training_data, training_label_matrix, opt, debug=False):
    """
    ...
    print(f'compute_tce_matrix(): vocabsize: {vocabsize}, opt.supervised: {opt.supervised}, opt.supervised_method: {opt.supervised_method}, opt.max_label_space: {opt.max_label_space}')
    ...
    TCE = get_supervised_embeddings(
        ...
        max_label_space=opt.max_label_space,
        #dozscore=(not opt.nozscore),
        dozscore=False,                         # no normalization for TCEs
        debug=debug
    )
    if (debug):
...
def get_tce_embeddings(texts_train, texts_val, texts_test, tokenizer, debug=False):
...
if __name__ == "__main__":
    ...
    #
    tce_matrix = None                       # initialize tce_matrix to None (not supervised)
    
    if (args.supervised):

        print("supervised is True, computing TCEs...")

        tce_matrix = get_tce_embeddings(
                        texts_train, 
                        texts_val, 
                        test_data, 
                        tokenizer, 
