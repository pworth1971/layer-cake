...
def main(opt):
    ...
    criterion = init_loss(dataset.classification_type, device=opt.device)
    print("criterion:\n", criterion)
    ...
    for epoch in range(1, opt.nepochs + 1):
        ...
        if opt.test_each>0:
            if (opt.plotmode and (epoch==1 or epoch%opt.test_each==0)) or (not opt.plotmode and epoch%opt.test_each==0 and epoch<opt.nepochs):
                test(model, test_index, yte, pad_index, dataset.classification_type, tinit, epoch, logfile, criterion, 'te', opt, embedding_size=emb_size_str)
    ...
    if not opt.plotmode:
        ...
        test(model, test_index, yte, pad_index, dataset.classification_type, tinit, epoch, logfile, criterion, 'final-te', opt, embedding_size=emb_size_str)
