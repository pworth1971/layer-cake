...
def test(model, test_index, yte, pad_index, classification_type, tinit, epoch, logfile, criterion, measure_prefix, opt):
    model.eval()
    predictions = []
...
def main(opt):
    ...
    model = init_Net(dataset.nC, vocabsize, pretrained_embeddings, sup_range, opt.device, opt)
    ...
    for epoch in range(1, opt.nepochs + 1):
        train(model, train_index, ytr, pad_index, tinit, logfile, criterion, optim, epoch, method_name, opt)

        # validation
        macrof1 = test(model, val_index, yval, pad_index, dataset.classification_type, tinit, epoch, logfile, criterion, 'va', opt)
        ...
        if opt.test_each>0:
            if (opt.plotmode and (epoch==1 or epoch%opt.test_each==0)) or (not opt.plotmode and epoch%opt.test_each==0 and epoch<opt.nepochs):
                test(model, test_index, yte, pad_index, dataset.classification_type, tinit, epoch, logfile, criterion, 'te', opt)
    ...
    if not opt.plotmode:
        ...
        if opt.val_epochs>0:
            ...
            for val_epoch in range(1, opt.val_epochs + 1):
                train(model, val_index, yval, pad_index, tinit, logfile, criterion, optim, epoch+val_epoch, method_name, opt)

        # test
        print('Training complete: testing')
        test(model, test_index, yte, pad_index, dataset.classification_type, tinit, epoch, logfile, criterion, 'final-te', opt)



if __name__ == '__main__':

    print("\n\t legacy layer cake modeling and testing (Neural text classification with Word-Class Embeddings)...")
    ...
    parser.add_argument('--patience', type=int, default=4, metavar='int',
                        help='patience for early-stop (default: 4)')
