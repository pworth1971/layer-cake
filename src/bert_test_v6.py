...
def train(model, train_loader, val_loader, device, class_type='single-label', epochs=3, learning_rate=2e-5, patience=3):
    ...
    for epoch in range(epochs):
        ...
        if (class_type in ['single-label', 'singlelabel']):
            ...
            print(f'Validation Macro F1: {val_macro_f1:.4f}, Validation Micro F1: {val_micro_f1}')
...
def validate(model, val_loader, device, class_type="single-label"):
    ...
    if class_type == "multi-label":
        ...
        all_labels = np.vstack(all_labels)  # Multi-label ground truth is 2D
    else:
        all_preds = np.array(all_preds)  # Single-label predictions are 1D
        all_labels = np.array(all_labels)  # Single-label ground truth is 1D
    ...
    if class_type == "multi-label":
        average = 'macro'  # You may also use 'samples' or 'weighted'
        macro_f1 = f1_score(all_labels, all_preds, average=average)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patience', type=int, default=2, metavar='int',
