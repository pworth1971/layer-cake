...
from data.lc_trans_dataset import LCTokenizer, get_vectorized_data, LCTFIDFVectorizer
...
class LCSequenceClassifier(nn.Module):
    ...
    def __init__(self, 
                ...
                relevant_token_ids: list = None,                                  # subset of relevant tokens for the dataset (NB this feature is not working)
        ...
        Args:
            ...
            relevant_token_ids: List of token ids to be considered relevant for classification computed from the underlying dataset.
        ...
        if relevant_token_ids is not None:
            ...
            #
            self.transformer, self.relevant_embeddings = self._filter_transformer_embeddings(hf_model=hf_model, relevant_token_ids=relevant_token_ids)                    
        else:
    ...
    def _filter_transformer_embeddings(self, hf_model, relevant_token_ids):
        """
        ...
        print(f"relevant_token_ids: {type(relevant_token_ids)} {len(relevant_token_ids)}")
        print("relevant_token_ids:[0]", type(relevant_token_ids[0]), relevant_token_ids[0])

        # Debugging: Check for None, NaN, and invalid values in relevant_token_ids
        invalid_values = [idx for idx in relevant_token_ids if idx is None or not isinstance(idx, int) or (isinstance(idx, float) and idx != idx)]
        if invalid_values:
            print(f"[ERROR] relevant_token_ids contains invalid values (e.g., None or NaN). Total count: {len(invalid_values)}")
            print(f"[ERROR] Full list of invalid values: {invalid_values}")
            raise ValueError("All relevant_token_ids must be integers and cannot contain None or NaN.")
        ...
        print("original_embeddings:[0]", type(original_embeddings[0]), original_embeddings[0].shape)
        
        # Validate relevant_token_ids are within range
        assert all(0 <= idx < original_embeddings.size(0) for idx in relevant_token_ids), (
            f"Some relevant_token_ids are out of range. "
            f"Valid range: [0, {original_embeddings.size(0) - 1}], "
            f"Received range: [{min(relevant_token_ids)}, {max(relevant_token_ids)}]"
        )

        # Get embeddings for relevant tokens
        relevant_embeddings = original_embeddings[relevant_token_ids, :]
        ...
        print("reduced_embedding_layer:", reduced_embedding_layer)
    
        hf_model.set_input_embeddings(reduced_embedding_layer)
        #print("updated hf_model:\n", hf_model)

        return hf_model, relevant_embeddings                                # class variables set on the return of params to calling funtion
...
def parse_args():
    ...
    parser.add_argument('--emb-filter', action='store_true', default=False,
                        help='filter model embeddings to align with dataset vocabulary (default False, the Classifier does not like this filtered Embedding layer for some reason)')
    """
...
if __name__ == "__main__":
    ...
    lc_model = LCSequenceClassifier(
        ...
        #relevant_token_ids=relevant_token_ids,          # relevant token_ids for the dataset
