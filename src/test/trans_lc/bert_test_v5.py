...
class CustomTokenizer:
    ...
    def __init__(self, model_name, max_length=512):
        """
        Custom tokenizer for use with TfidfVectorizer.
        
        Args:
            model_name (str): Pretrained model name or path (e.g., 'bert-base-uncased').
    ...
    def __call__(self, text):
        """
        Tokenize the text and truncate to the maximum length.
    ...
    def tokenize(self, text):
        """
        Alias for the __call__ method to maintain compatibility.
        """
...
def vectorize(texts_train, texts_val, texts_test, tokenizer, vtype='tfidf'):
    ...
    print(f'vectorize(), vtype: {vtype}, tokenizer max_length: {max_length}')

    # Helper function to tokenize and truncate
    def tokenize_and_truncate(text):
        tokens = tokenizer.tokenize(text)  # Tokenize text
        truncated_tokens = tokens[:max_length]  # Truncate to max_length
        return " ".join(truncated_tokens)  # Convert tokens back to string

    # Preprocess texts for vectorizer
    tokenized_texts_train = [tokenize_and_truncate(text) for text in texts_train]
    tokenized_texts_val = [tokenize_and_truncate(text) for text in texts_val]
    tokenized_texts_test = [tokenize_and_truncate(text) for text in texts_test]
    ...
    Xtr = vectorizer.fit_transform(tokenized_texts_train)
    Xval = vectorizer.transform(tokenized_texts_val)
    Xte = vectorizer.transform(tokenized_texts_test)
...
class TextDataset(Dataset):
    ...
    def __init__(self, encodings, labels, vectorizer_vocab, tokenizer, max_length, class_type='multi-label'):
...
def get_tokenizer_and_model(model_name: str, model_path: str = None, max_length: int = 512):
    """
    Returns the tokenizer and model for the specified LLM, with a custom tokenizer wrapper.

    Args:
        model_name (str): Name of the pretrained LLM. Supported values:
                          models containing 'bert', 'roberta', 'distilbert', 'xlnet', 'gpt2', or 'llama'.
        model_path (str): Path to the pretrained model directory. If None, it will download from the Hugging Face hub.
        max_length (int): Maximum token length for truncation.

    Returns:
        custom_tokenizer: The custom tokenizer for the specified LLM.
        model: The model for the specified LLM.
    """

    print("get_tokenizer_and_model...model_name:", model_name, "model_path:", model_path)

    if "bert" in model_name.lower():
        tokenizer = BertTokenizerFast.from_pretrained(model_name, cache_dir=model_path)
        model = BertModel.from_pretrained(model_name, cache_dir=model_path)
    elif "roberta" in model_name.lower():
        tokenizer = RobertaTokenizerFast.from_pretrained(model_name, cache_dir=model_path)
        model = RobertaModel.from_pretrained(model_name, cache_dir=model_path)
    elif "distilbert" in model_name.lower():
        tokenizer = DistilBertTokenizerFast.from_pretrained(model_name, cache_dir=model_path)
        model = DistilBertModel.from_pretrained(model_name, cache_dir=model_path)
    elif "xlnet" in model_name.lower():
        tokenizer = XLNetTokenizerFast.from_pretrained(model_name, cache_dir=model_path)
        model = XLNetModel.from_pretrained(model_name, cache_dir=model_path)
    elif "gpt2" in model_name.lower():
        tokenizer = GPT2TokenizerFast.from_pretrained(model_name, cache_dir=model_path)
        model = GPT2Model.from_pretrained(model_name, cache_dir=model_path)
        tokenizer.pad_token = tokenizer.eos_token  # GPT-2 requires padding token for compatibility in some tasks
    elif "llama" in model_name.lower():
        tokenizer = LlamaTokenizer.from_pretrained(model_name, cache_dir=model_path)
        model = LlamaModel.from_pretrained(model_name, cache_dir=model_path)
    else:
        raise ValueError(f"Unsupported model_name: {model_name}. Supported values are 'bert', 'roberta', "
                         "'distilbert', 'xlnet', 'gpt2', and 'llama'.")
...
def classify(opt, device, batch_size=MPS_BATCH_SIZE, epochs=EPOCHS, max_length=MAX_LENGTH, model_name='bert-base-uncased'):
    ...
    print(f'classify...opt: {opt}, device: {device}, batch_size: {batch_size}, epochs: {epochs}, max_length: {max_length}, model_name: {model_name}')
    ...
    tokenizer, transformer_model = get_tokenizer_and_model(model_name, emb_path)
    transformer_model.to(device)    
    print("Initialized tokenizer:\n", tokenizer)
    print("Initialized transformer model:\n", transformer_model)

    def preprocess_texts_for_tokenizer(texts, tokenizer):
        """
        Tokenizes and truncates input texts to ensure compatibility with the Hugging Face tokenizer's max_length.
        ...
        Returns:
            list: Preprocessed texts, truncated to tokenizer's max_length.
        """
        max_length = tokenizer.model_max_length

        def tokenize_and_truncate(text):
            tokens = tokenizer.tokenize(text)  # Tokenize text
            truncated_tokens = tokens[:max_length]  # Truncate tokens
            return tokenizer.convert_tokens_to_string(truncated_tokens)  # Convert tokens back to text

        return [tokenize_and_truncate(text) for text in texts]

    # Preprocess texts for consistent truncation
    texts_train_preprocessed = preprocess_texts_for_tokenizer(texts_train, tokenizer)
    texts_val_preprocessed = preprocess_texts_for_tokenizer(texts_val, tokenizer)
    texts_test_preprocessed = preprocess_texts_for_tokenizer(test_data, tokenizer)

    # Tokenize preprocessed texts
    train_encodings = tokenizer(
        texts_train_preprocessed,
        truncation=True,  # Truncation is now a safeguard
        padding=True,
        max_length=tokenizer.model_max_length,
        return_tensors='pt'
    )
    val_encodings = tokenizer(
        texts_val_preprocessed,
        truncation=True,
        padding=True,
        max_length=tokenizer.model_max_length,
        return_tensors='pt'
...
if __name__ == '__main__':
    ...
    model_name = get_model_identifier(args.pretrained)
    ...
    if (args.pretrained == 'bert'):
        ...
        model_path = args.distilbert_path
    elif (args.pretrained == 'xlnet'):
        model_path = args.xlnet_path
    elif (args.pretrained == 'gpt2'):
        model_path = args.gpt2_path
