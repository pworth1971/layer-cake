...
def vectorize(texts_train, texts_val, texts_test, tokenizer, vtype):
    ...
    print(f'vectorize(), using tokenizer vocab, vtype: {vtype}; tokenizer: {tokenizer}')
...
def embedding_matrix(model, tokenizer, vocabsize, word2index, out_of_vocabulary, vectorized_training_data, training_label_matrix, opt):
    ...
    # Populate pretrained_matrix with pretrained model embeddings
    if opt.pretrained:
        for word, idx in word2index.items():
            token_id = tokenizer.convert_tokens_to_ids(word)
            if token_id is not None and token_id < embedding_layer.num_embeddings:
...
class LCTransformerClassifier(nn.Module):
    ...
    def __init__(self, transformer_model_name, output_dim, embeddings, embedding_dim, mode='single-label'):
        """
        Initializes the LCTransformerClassifier.

        Args:
        - transformer_model_name: Hugging Face transformer model name.
        - output_dim: Number of output classes.
        - embeddings: The embedding matrix (pretrained or combined pretrained + supervised).
        - embedding_dim: Total embedding dimensions (e.g., 768 for pretrained or 768 + supervised dimensions).
        - mode: Classification mode ('single-label' or 'multi-label').
        """
        ...
        # Replace the transformer's word embedding layer with combined embeddings
        self.transformer.embeddings.word_embeddings = nn.Embedding.from_pretrained(embeddings, freeze=False)

        # If embedding_dim differs from the transformer's hidden size, add a projection layer
        transformer_hidden_size = self.transformer.config.hidden_size
        if self.embedding_dim != transformer_hidden_size:
            self.projection_layer = nn.Linear(self.embedding_dim, transformer_hidden_size)
        else:
            self.projection_layer = None

        # Update position embeddings and LayerNorm if dimensions differ
        if self.embedding_dim != self.transformer.config.hidden_size:
            self.transformer.embeddings.position_embeddings = nn.Embedding(
                self.transformer.embeddings.position_embeddings.num_embeddings,
                self.embedding_dim,
            )
            self.transformer.embeddings.LayerNorm = nn.LayerNorm(self.embedding_dim, eps=1e-12)

            # Update transformer hidden size in the config
            #self.transformer.config.hidden_size = self.embedding_dim

        # Classification output layer
        self.output_layer = nn.Linear(transformer_hidden_size, output_dim)
        
        if (self.mode in ['single-label', 'singlelabel']):
            self.activation = nn.Softmax(dim=-1)
            self.loss_fn = nn.CrossEntropyLoss()                # Cross-Entropy for single-label
        elif (self.mode in ['multi-label', 'multilabel']):
    ...
    def forward(self, input_ids, attention_mask, labels=None, embeddings=None):
        ...
        Args:
        - input_ids: Token IDs for the transformer (batch_size, seq_len).
        - attention_mask: Attention mask for the transformer (batch_size, seq_len).
        - labels: Ground-truth labels for loss computation (optional).

        Returns:
        - A dictionary containing logits and optionally the loss if labels are provided.
        """

        """
        # Get CLS embedding from the transformer
...
class LCDataset(Dataset):
    ...
    def __init__(self, texts, labels, tokenizer, max_length=MAX_LENGTH, class_type='single-label', embeddings=None):
        ...
        - embeddings: embeddings which include pretrained + WCEs et al
        """
        self.texts = texts
        self.labels = labels
    ...
    def __getitem__(self, idx):
        """
        Fetch an individual sample from the dataset.

        Returns:
        - item: Dictionary containing tokenized inputs, labels, and optional supervised embeddings.
        """
        text = self.texts[idx]
...
def custom_data_collator(batch):
    """
    Custom data collator for handling variable-length sequences, labels,
    and optional supervised embeddings.

    Parameters:
    - batch: List of individual samples from the dataset.

    Returns:
    - collated: Dictionary containing collated input_ids, attention_mask, labels, and embeddings.
    """
    collated = {
        ...
        "embeddings": torch.stack([f["embeddings"] for f in batch])
    }
...
if __name__ == "__main__":
    ...
    onehot_labels_train = oh_encoder.fit_transform(labels_train.reshape(-1, 1))
    print("onehot_lebls_train:", type(onehot_labels_train), onehot_labels_train.shape)
    print("onehot_lebls_train[0]:", type(onehot_labels_train[0]), onehot_labels_train[0].shape, onehot_labels_train[0])

    # Call `embedding_matrix` with the loaded model and tokenizer
    """
    pt_embeddings, wces, sup_range = embedding_matrices(
        model=hf_model,
        tokenizer=tokenizer,
        ...
        training_label_matrix=onehot_labels_train,
        opt=args
    )
    ...
    pt_embeddings, sup_range, embedding_dim = embedding_matrix(
        model=hf_model,
        tokenizer=tokenizer,
        vocabsize=vec_vocab_size,
        word2index=vectorizer.vocabulary_,
    ...
    lc_model = LCTransformerClassifier(
        transformer_model_name=model_name,              # Hugging Face model name
        output_dim=len(target_names),
        embeddings=pt_embeddings,
        embedding_dim=embedding_dim,
        mode=class_type                                 # 'multi-label' or 'single-label'
    ).to(device)
    print("lc_model:\n", lc_model)
    ...
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure=f'{measure_prefix}-macro-F1', value=macrof1, timelapse=tend)
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure=f'{measure_prefix}-micro-F1', value=microf1, timelapse=tend)
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure=f'{measure_prefix}-accuracy', value=acc, timelapse=tend)
    #logfile.insert(dimensions=embedding_size, epoch=epoch, measure=f'{measure_prefix}-loss', value=loss, timelapse=tend)

    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure='te-hamming-loss', value=h_loss, timelapse=tend)
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure='te-precision', value=precision, timelapse=tend)
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure='te-recall', value=recall, timelapse=tend)
    logfile.insert(dimensions=pt_embeddings.shape, epoch=epoch, measure='te-jacard-index', value=j_index, timelapse=tend)
