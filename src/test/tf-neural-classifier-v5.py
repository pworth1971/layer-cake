...
from tensorflow.keras.layers import Dense, Conv1D, GlobalMaxPooling1D, Attention, Bidirectional
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Embedding, LSTM, Input
...
VECTOR_CACHE = '../../layer-cake/.vector_cache/'
IMDB_DATA_DIR = '../datasets/IMDB/'
GLOVE_DIR = os.path.join(VECTOR_CACHE, 'GloVe')

print("GLOVE_DIR: ", GLOVE_DIR)
print("IMDB_DATA_DIR: ", IMDB_DATA_DIR)
...
def load_imdb_data_nn(max_words=MAX_WORDS, max_sequence_length=MAX_SEQUENCE_LENGTH, debug=False):

  print(f'loading imdb data... max_words={max_words}, max_sequence_length={max_sequence_length}')
  ...
  return train_data, x_train, y_train, train_labels, x_val, y_val, test_data, test_texts, y_test, test_labels, num_classes, class_names



def load_20news_data_nn(max_words=MAX_WORDS, max_sequence_length=MAX_SEQUENCE_LENGTH, debug=False):
    
  print(f'loading 20 newsgroups data... max_words={max_words}, max_sequence_length={max_sequence_length}')
...
def load_vectorized_reuters_data(vtype="count", max_features=MAX_WORDS, debug=False):
...
def load_vectorized_20newsgroups_data(vtype='count', max_features=MAX_WORDS, debug=False):
...
def main(args):
    ...
    print("max_words:", args.max_words)

    if (args.dataset == 'imdb'):

      print("loading and prepping IMDB dataset....")

      train_data, x_train, y_train, train_labels, x_val, y_val, test_data, x_test, y_test, test_labels, num_classes, \
        class_names = load_imdb_data_nn(max_words=args.max_words, debug=args.debug)
      ...
      if (args.model_type == 'dense'):
        ...
        print("dense model, vectorizng text...")

        raw_train, x_train_vectorized, y_train, raw_test, x_test_vectorized, y_test, train_labels, test_labels, \
          vectorizer = load_vectorized_reuters_data(vtype=args.vtype, max_features=args.max_words, debug=args.debug)

      elif (args.model_type == 'cnn'):
          
        x_train, y_train, train_labels, x_val, y_val, x_test, y_test, test_labels, class_names, \
          word_index = load_reuters_data_nn(max_words=args.max_words, debug=False) 

      else:
          raise ValueError(f"Unsupported model type: {args.model_type}, for dataset: {args.dataset}")           
      ...
      if (args.model_type == 'dense'):
        ...
        raw_train, x_train_vectorized, y_train, raw_test, x_test_vectorized, y_test, train_labels, test_labels, class_names, \
            vectorizer = load_vectorized_20newsgroups_data(vtype=args.vtype, max_features=args.max_words, debug=args.debug)
        ...
        train_data, x_train, y_train, train_labels, x_val, y_val, test_data, test_texts, y_test, test_labels, num_classes, \
          class_names = load_20news_data_nn(max_words=args.max_words, debug=False)            
    ...
    if (args.model_type == 'dense'):
        ...
        print("\n\t...building and fitting cnn model...")
        ...
        print("model.summary():", cnnmodel.summary())
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="imdb", choices=["reuters", "20newsgroups", "imdb"], help="Dataset to use (reuters, 20newsgroups, imdb, etc.)")
